...
//Desafio 1 - identificar qual data que os servicos foram realizados
function darBanhoNoPet(pet){
  let data = new Date();
  data = data.getDate() + "/" + (data.getMonth() + 1) + "/" + data.getFullYear();
  pet.servicos.push(data);
  console.log("O pet " + pet.nome + " tomou banho em " + data);
}
...
function tosarPet(pet){
  let data = new Date();
   data = data.getDate() + "/" + (data.getMonth() + 1) + "/" + data.getFullYear();
  pet.servicos.push(data);
  console.log("O pet " + pet.nome + " foi tosado em " + data);
}
 //servicosPrestados(pets[1],darBanhoNoPet);
// servicosPrestados(pets[1],tosarPet);
...
// Desafio 2 - Crie uma função filtrarPetPorNome, que quando receber um parâmetro que será uma string
